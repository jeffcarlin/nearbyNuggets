...
def cutout_coadd(butler, ra, dec, band='r', datasetType='deepCoadd_calexp',
...
def plotcand6_butler(butler, sc_inp, cat, nsig,
                     comp_flag, cmdsel_flag,
                     star_flag,
                     binsize=1.5, recalc_cen=False, savefig=False,
                     name_append='', overlay_pts=False):
    ...
    ----------
    butler :
        Butler containing the images and catalogs
    sc_inp : `astropy.skycoord`
        input central coordinates of the candidate's position
    ###sc_all : `astropy.skycoord`
        ###input coordinates of all objects
...
# add RGB box corners as input params
    ...
    # gcoadd_refs = butler.query_datasets('deepCoadd_calexp',
    #                                     where="patch.region OVERLAPS POINT(ra, dec) AND band='g'",
    #                                     bind={'ra': censtar.ra.value, 'dec': censtar.dec.value})
    # icoadd_refs = butler.query_datasets('deepCoadd_calexp',
    #                                     where="patch.region OVERLAPS POINT(ra, dec) AND band='i'",
    #                                     bind={'ra': censtar.ra.value, 'dec': censtar.dec.value})

    # tract = gcoadd_refs[0].dataId['tract']
    # patch = gcoadd_refs[0].dataId['patch']
    ...
    zscale = ZScaleInterval()
    vmin_g, vmax_g = zscale.get_limits(cutout_g.image.array)
    vmin_r, vmax_r = zscale.get_limits(cutout_r.image.array)
    vmin_i, vmax_i = zscale.get_limits(cutout_i.image.array)

    img = np.zeros((cutout_i.image.array.shape[0], cutout_i.image.array.shape[1], 3), dtype=float)
    img[:, :, 0] = img_scale.linear(cutout_i.image.array, scale_min=0.1*vmin_i, scale_max=0.6*vmax_i)
    img[:, :, 1] = img_scale.linear(cutout_r.image.array, scale_min=0.1*vmin_r, scale_max=0.9*vmax_r)
    img[:, :, 2] = img_scale.linear(cutout_g.image.array, scale_min=0.1*vmin_g, scale_max=0.75*vmax_g)
    ...
    wcs = cutout_g.wcs
    x_rgb, y_rgb = wcs.skyToPixelArray(ra[cand_flag & star_flag & cmdsel_flag],
                                       dec[cand_flag & star_flag & cmdsel_flag],
                                       degrees=True)
    ...
    if overlay_pts:
        fig.plot(x_rgb-bbox_g.getBeginX(), y_rgb-bbox_g.getBeginY(), 'ws', ms=10,
                 markeredgewidth=1, fillstyle='none')
